...
4. "keywords": Un arreglo de strings donde CADA STRING CORRESPONDE A LA BÚSQUEDA EXACTA DEL ÍTEM SOLICITADO en un motor de búsqueda como MercadoLibre o Google.
   REGLA DE ORO PARA KEYWORDS: 
   - Debes extraer MÁXIMO 3 a 4 palabras clave altamente comerciales para cada ítem. NUNCA incluyas descripciones largas, certificaciones, voltajes secundarios ni frases completas.
   - Ejemplos correctos: "UPS 800W 220V", "Notebook i7 16GB", "Resma Carta 500 hojas".
   - Ejemplos INCORRECTOS: "UPS 375 VA 800W 220V 12V 9Ah interactiva tipo torre 1 entrada 4 salidas" (demasiado largo, no arrojará resultados comerciales).
   - Omite palabras genéricas burocráticas (ej. "adquisición", "compra", "suministro", "requiere").
...
Devuelve estrictamente un objeto JSON válido y nada más. Ejemplo de salida asumiendo que la licitación pide 2 ítems:
{
  ...
  "keywords": ["Impresora HP M404dw", "Tóner HP M404dw"]
}`;
            const contents = [
...
export async function extractKeywordsForItems(tenderName, items) {
    if (!process.env.GEMINI_API_KEY) {
        // Fallback heurístico simple
        const fallback = {};
        items.forEach((item, idx) => { fallback[idx] = item.nombre; });
        return fallback;
    }
    ...
    while (retries > 0) {
        try {
            ...
            const systemPrompt = `Eres un experto en compras públicas y e-commerce.
Tu tarea es descubrir el verdadero producto comercial que se quiere comprar, ignorando categorías genéricas.
En el gobierno, los compradores suelen llenar el "Nombre" con categorías inútiles (ej. "Notebook, laptop o computador portatil excepto Tablet PC"), pero detallan el producto real en la "Descripción" (ej. "1 NOTEBOOK PROCESADOR i 5 O RYSEN 7 ALMACENAMIENTO SSD DE 512 GB WINDOW 11 PROFESIONAL MEMORIA 8 GB").

REGLAS CRÍTICAS:
1. IGNORA EL "NOMBRE" SI ES GENÉRICO. La "Descripción" MANDA y contiene las especificaciones técnicas reales que debemos buscar.
2. Extrae las especificaciones CLAVE de la descripción (ej. "Notebook i5 8GB 512GB SSD").
3. El término final debe ser apto para Mercado Libre o Google (máximo 5-7 palabras). No pongas toda la descripción, solo lo fundamental para encontrar el producto.
4. Omite frases como "Oficina", "Hogar y Empresa", "1 unidad", "Color negro". Concéntrate en la marca, modelo, material o especificación técnica principal.

Licitación general: "${tenderName}"
Ítems a analizar:
${items.map((it, idx) => `Ítem ${idx}:\nNombre: ${it.nombre}\nDescripción: ${it.descripcion}\n---`).join('\n')}

Debes devolver UNICAMENTE un JSON que sea un array de objetos. Cada objeto debe tener:
- "index": El número del ítem (0, 1, 2...)
- "razonamiento": Breve explicación de por qué elegiste el término.
- "termino": El término de búsqueda optimizado para Mercado Libre.`;
            ...
            let jsonText = response.text.trim();
            console.log("GEMINI RAW:", jsonText);
            ...
            if (Array.isArray(parsedArray)) {
                parsedArray.forEach(item => {
                    if (item.index !== undefined && item.termino) {
                        resultObj[item.index.toString()] = item.termino;
                    }
                });
            } else {
                // Si por alguna razón devolvió un objeto directo
                return parsedArray;
            }
            return resultObj;
        } catch (e) {
            console.error(`Error extractKeywordsForItems (Retries left: ${retries - 1}):`, e.message);
            retries--;
    ...
    // Fallback if fails
    throw new Error("extractKeywordsForItems failed after all retries. Quota might be exhausted.");
}
