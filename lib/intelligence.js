...
export async function extractKeywordsForItems(tenderName, items) {
    ...
    while (retries > 0) {
        try {
            ...
            const systemPrompt = `Eres un experto en compras públicas y comercio electrónico.
Se te entregará el nombre de una licitación y una lista de ítems solicitados (con nombre y descripción).
Tu objetivo es devolver un objeto JSON donde cada llave sea el índice del ítem (0, 1, 2...) y el valor sea el término de búsqueda MÁS ÓPTIMO para buscar ese producto exacto en Mercado Libre.
REGLAS:
- Extrae la esencia del producto (ej. "Rollo de pasto sintético", "Manguera jardín 1/2").
- Si el ítem es genérico (ej: "Mejora de pastos"), deduce el producto real de la descripción.
- Máximo 4-5 palabras por término.
- No uses palabras como "adquisición", "suministro", "requiere".
Licitación: "${tenderName}"
Ítems:
${items.map((it, idx) => `[${idx}] Nombre: ${it.nombre} | Desc: ${it.descripcion}`).join('\n')}

Devuelve SOLO un objeto JSON válido, ej: {"0": "Rollo pasto sintético", "1": "Manguera jardín 1/2"}`;
